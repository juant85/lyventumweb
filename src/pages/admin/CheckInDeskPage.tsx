// src/pages/admin/CheckInDeskPage.tsx
...
const AttendeeRow: React.FC<{
  ...
  return (
    <div className={`p-3 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-colors duration-200 ${isCheckedIn ? 'bg-green-50 dark:bg-green-900/30' : 'bg-white dark:bg-slate-800'}`}>
      <div className="flex items-center min-w-0 flex-grow w-full">
        {attendee.avatar_url ? (
          <img src={attendee.avatar_url} alt={attendee.name} className="w-10 h-10 rounded-full mr-3 flex-shrink-0 object-cover" />
        ) : (
          <UserIcon className={`w-10 h-10 p-1.5 rounded-full mr-3 flex-shrink-0 ${isCheckedIn ? 'bg-green-100 text-green-500' : 'bg-slate-100 dark:bg-slate-700 text-slate-400'}`} />
        )}
        <div className="min-w-0 flex-grow">
          <button onClick={() => onOpenProfile(attendee)} className="text-left hover:underline focus:outline-none focus:ring-1 focus:ring-primary-500 rounded-sm px-1 -ml-1">
            <p className="font-semibold text-slate-800 dark:text-slate-100 truncate" title={attendee.name}>{attendee.name}</p>
          </button>
          <div className="flex items-center gap-2">
            <p className="text-sm text-slate-500 dark:text-slate-400 truncate" title={attendee.organization}>{attendee.organization || 'No organization'}</p>
            {attendee.is_vendor && (
              <span className="bg-purple-100 text-purple-800 text-xs font-semibold px-2 py-0.5 rounded-full dark:bg-purple-900 dark:text-purple-300">VENDOR</span>
            )}
          </div>
        </div>
      </div>
      <div className="flex items-center flex-shrink-0 w-full sm:w-auto justify-end pl-12 sm:pl-0">
        <button
          onClick={() => onOpenDetails(attendee)}
          className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
          title={attendee.notes ? t(localeKeys.editDetailsButtonTitle) : t(localeKeys.addDetailsButtonTitle)}
          disabled={isProcessing}
        >
          <DocumentTextIcon className={`w-5 h-5 ${attendee.notes ? 'text-primary-500' : 'text-slate-400'}`} />
        </button>
        {canTakePhoto && !attendee.avatar_url && (
          <button
            onClick={() => onTakePhoto(attendee)}
            className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors ml-1"
            title={t(localeKeys.takePhoto)}
            disabled={isProcessing}
          >
        ...
        {isCheckedIn ? (
          <div className="text-right w-28 ml-2">
            <p className="text-xs font-bold text-green-600 dark:text-green-400">{t(localeKeys.checkedInStatus).toUpperCase()}</p>
...
const CheckInDeskPage: React.FC = () => {
  ...
  const handleCheckIn = async (id: string, name: string) => {
    ...
    if (result.success) {
      ...
      // ✨ NEW: Auto-send access code
      try {
        const attendee = attendees.find(a => a.id === id);
        if (attendee && attendee.email && selectedEventId) {
          ...
          toast.success(`📧 Access code sent to ${attendee.email}`);
        }
      } catch (err) {
        console.error('Error sending access code:', err);
        // Don't block check-in flow
        toast.error('⚠️ Access code sending failed. Please resend manually.');
      }
    } else {
    ...
    if (loading) {
      return (
        <div className="text-center py-10">
          ...
          <p className="mt-2 text-slate-500">{t(localeKeys.loadingAttendeesFor, { eventName: currentEvent?.name || '...' })}</p>
        </div>
      );
    }
    if (error) {
  ...
  return (
    <>
      ...
      <Modal isOpen={!!selectedProfile} onClose={handleCloseModals} title={t(localeKeys.profileQuickViewTitle)} size="sm">
        {selectedProfile && (
          <div className="space-y-4">
            ...
            >
              {t(localeKeys.goToFullProfileButton)}
            </Button>
          </div>
        )}
      </Modal>
      ...
      {/* Edit Details Modal */}
      <Modal isOpen={!!selectedAttendeeForEdit} onClose={handleCloseModals} title={t(localeKeys.editDetailsTitle, { name: selectedAttendeeForEdit?.name || '' })} size="lg">
        {selectedAttendeeForEdit && (
          <div className="space-y-4">
            <div>
              <label htmlFor="attendee-notes" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 font-montserrat">{t(localeKeys.notesLabel)}</label>
              <textarea
                id="attendee-notes"
                value={currentNotes}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 font-montserrat">{t(localeKeys.preferencesLabel)}</label>
            ...
            <div className="flex justify-end gap-3 pt-4">
              <Button variant="neutral" onClick={handleCloseModals} disabled={isSavingDetails}>{t(localeKeys.cancel)}</Button>
              <Button variant="primary" onClick={handleSaveDetails} disabled={isSavingDetails}>
                {isSavingDetails ? t(localeKeys.saving) : t(localeKeys.saveChanges)}
              </Button>
            </div>
          </div>
        )}
      ...
      {/* Add Attendee Modal */}
      <Modal isOpen={isAddModalOpen} onClose={handleCloseModals} title={t(localeKeys.addWalkInTitle)} size="md">
        <form onSubmit={handleAddNewAttendee} className="space-y-4">
          ...
          <Input
            label={t(localeKeys.nameRequired)}
          ...
          />
          <Input
            label={t(localeKeys.orgRequired)}
          ...
          />
          <Input
            label={t(localeKeys.emailRequiredLabel)}
            type="email"
          ...
          />
          <Input
            label={t(localeKeys.contactPhone)}
          ...
          <div>
            <label htmlFor="walkin-notes" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 font-montserrat">{t(localeKeys.notesLabel)}</label>
            <textarea
              id="walkin-notes"
          ...
          <div className="flex justify-end gap-3 pt-4">
            <Button type="button" variant="neutral" onClick={handleCloseModals} disabled={isAdding}>{t(localeKeys.cancel)}</Button>
            <Button type="submit" variant="primary" disabled={isAdding}>
              {isAdding ? t(localeKeys.adding) : t(localeKeys.saveAndCheckIn)}
            </Button>
          </div>
        </form>
      </Modal>
      ...
      <div className="space-y-6">
        <h1 className="text-3xl font-bold text-gray-800 dark:text-slate-100 flex items-center font-montserrat">
          ...
          {t(localeKeys.checkInDeskTitle)}
        </h1>

        <Card>
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4 border-b border-slate-200 dark:border-slate-700 pb-4">
            <Input
              id="attendee-search"
              placeholder={t(localeKeys.searchAttendeesPlaceholder)}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              disabled={loading || !!error || !selectedEventId || attendees.length === 0}
              wrapperClassName="!mb-0 flex-grow w-full md:w-auto"
            />
            <div className="flex items-center gap-4 w-full md:w-auto">
              <Button
                ...
                className="bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 flex-grow md:flex-grow-0"
              >
                {t(localeKeys.addNewAttendeeButton)}
              </Button>
              <div className="text-center md:text-right bg-slate-100 dark:bg-slate-700 p-3 rounded-lg flex-grow md:flex-grow-0">
                <p className="text-sm font-semibold text-slate-500 dark:text-slate-300">{t(localeKeys.attendanceHeader)}</p>
