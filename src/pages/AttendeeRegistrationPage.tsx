...
import { UsersGroupIcon, ArrowPathIcon, TableCellsIcon, TrashIcon, PlusCircleIcon } from '../components/Icons';
...
const AttendeeRegistrationPage: React.FC = () => {
    const { findOrCreateAttendeesBatch, loadingData: eventDataLoading, fetchData } = useEventData();
    ...
    const handleConfirmImport = async () => {
        ...
        const attendeesToProcess = editableData.map(row => {
            const customData: { [key: string]: any } = {};

            reviewColumns.forEach(col => {
                const value = row[col.key];
                if (!KNOWN_FIELDS.some(f => f.value === col.key)) {
        ...
        toast.loading(`Importing ${attendeesToProcess.length} attendees...`, { id: toastId });
        ...
        if (result.success) {
            ...
            toast.error(`Import failed. ${result.errors.join('. ')}`, { id: toastId });
    ...
    return (
        <div className="space-y-6">
            <h1 className="text-3xl font-bold flex items-center font-montserrat">
                <UsersGroupIcon className="w-8 h-8 mr-3 text-brandBlue" /> <span className="text-gradient">Flexible Attendee Import</span>
            </h1>
            ...
            <Card title="Step 1: Upload Attendee File" className="shadow-lg glass-card">
                <p className="text-sm text-gray-600 dark:text-slate-300 mb-4">Upload an Excel (.xlsx, .xls) or CSV file. The first row must be headers.</p>
                <Input
