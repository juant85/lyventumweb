...
const LyVentumLogo = ({ className = '', variant = 'gradient' }: { className?: string, variant?: 'gradient' | 'light' }) => {
  ...
  const textGradientId = `lyventumTextGradient-${uniqueId}`;
  const barsGradientId = `lyventumBarsGradient-${uniqueId}`;
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={textGradientId} x1="0" y1="0.5" x2="1" y2="0.5">
          <stop offset="0%" stopColor="#004aad" />
          <stop offset="50%" stopColor="#0076ce" />
          <stop offset="100%" stopColor="#00b9d8" />
        </linearGradient>
        <linearGradient id={barsGradientId} x1="0.5" y1="0" x2="0.5" y2="1">
      ...
      <path
        ...
        fill={isLight ? 'white' : `url(#${textGradientId})`}
      />

      {/* Momentum Bars */}
      <g fill={isLight ? 'white' : `url(#${barsGradientId})`}>
